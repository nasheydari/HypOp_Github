...
import torch
import timeit
from src.loss import loss_sat_numpy, loss_maxcut_numpy, loss_maxcut_numpy, loss_maxind_numpy, loss_maxind_QUBO,  loss_task_numpy_vec
import random
...
def mapping_distribution_vec_task(best_outs, params, n, info, weights, constraints, C_dic, all_weights, inc, lenc, leninfo, penalty, hyper):
    if params['random_init']=='one_half':
        best_outs= {x: 0.5 for x in best_outs.keys()}
    elif params['random_init']=='uniform':
        best_outs = {x: np.random.uniform(0,1) for x in best_outs.keys()}
